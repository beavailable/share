...
class BaseHandler(BaseHTTPRequestHandler):
    ...
    def _build_html_for_password(self):
        ...
        builder.start_head()
        builder.start_title()
        builder.append(self._hostname+"分享的文件")
        builder.end_title()
        builder.start_style()
        ...
        builder.append('<input name="password" type="password" placeholder="请输入密码，提取文件" minlength="3" style="border-radius: 30px;border: 1px solid #d6d4d4;height:30px;text-indent:10px"required autofocus>')
        builder.append('&nbsp<input type="submit"background-color: #00f;">')
        builder.append('</form>')
        builder.append('</div>')
        builder.end_body()
...
class BaseFileShareHandler(BaseHandler):
    ...
    def build_html(self, path, dirs, files):
        ...
        builder.append('.header{display: flex; justify-content: space-between; padding: 10px 0; font-size: xx-large;}')
        builder.append('hr{width: 100%;}')
        builder.append('.main{flex: auto; padding: 16px 0;}')
        builder.append('.content{width: 100%; height: 100%;}')
        builder.append('.list-item{display: flex; justify-content: space-between; padding: 4px 0; word-break: break-all;}')
        ...
        builder.append('function view_file(e){')
        builder.append('    e.preventDefault();')
        builder.append('    src = this.getAttribute("href");')
        ...
        builder.append('<div>')
        builder.append(f'<a href="/">{html.escape(self._hostname)+"分享的文件"}</a>')
        p = ''
        ...
        builder.append('</div>')
        if self._upload:
            builder.append('<button id="upload" class="upload">上传文件</button>')
        ...
        for f, hidden, size in files:
            ...
            builder.append('<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" height="20px" viewBox="0 0 24 24" width="20px" fill="#2965c7"><g><rect fill="none" height="20" width="20"/></g><g><path d="M18,15v3H6v-3H4v3c0,1.1,0.9,2,2,2h12c1.1,0,2-0.9,2-2v-3H18z M17,11l-1.41-1.41L13,12.17V4h-2v8.17L8.41,9.59L7,11l5,5 L17,11z"/></g></svg>')
            builder.append('</a>')
            builder.append('</span>')
            builder.append('</li>')
        builder.append('</ul>')
...
class TextReceiveHandler(BaseHandler):
    ...
    def build_html(self):
        ...
        builder.start_head()
        builder.start_title()
        builder.append("请输入文本 -"+self._hostname)
        builder.end_title()
        builder.start_style()
        builder.append('.container{height: 100%; display: flex; flex-direction: column;}')
        ...
        builder.append('<textarea id="text" class="textarea" name="text" placeholder="输入文本" required autofocus></textarea>')
        builder.append('<br>')
...
class HtmlBuilder:
    ...
    def start_style(self):
        ...
        self._out.write('body{width: 55vw;}')
        self._out.write('}')
...
def print_prompt():
    if is_windows():
        sys.stderr.write('输入文本，按 Ctrl + Z，然后按 Enter 键：\n')
    else:
        sys.stderr.write('输入文本，按ctrl+d:\n')
...
def start_server(address, port, certfile, keyfile, keypass, handler_class):
    ...
    with ShareServer(addr, handler_class) as server:
        ...
        sys.stderr.write(f'在 {host} 端口 {port} 上提供 HTTP 服务 ...\n')
...
def main():
    ...
    general.add_argument('-b', '--bind', dest='address', help='绑定IP地址 [默认: all interfaces]')
    general.add_argument('-p', '--port', type=int, default=8888, help='端口号 [默认: 8888]')
    general.add_argument('-s', '--share', action='store_true', help='分享模式 (默认模式)')
    general.add_argument('-r', '--receive', action='store_true', help='接收模式, 可与 -s 选项一起使用（仅适用于目录）')
    general.add_argument('-a', '--all', action='store_true', help='仅显示目录的所有文件，包括隐藏的文件')
    general.add_argument('-t', '--text', action='store_true', help='文本')
    general.add_argument('-P', '--password', nargs='?', const=os.getenv('SHARE_PASSWORD'), help='访问密码，如果未指定密码，则使用环境变量SHARE_PASSWORD')
    general.add_argument('-h', '--help', action='help', help='显示此帮助消息并退出')

    tls = parser.add_argument_group('tls选项')
    tls.add_argument('--certfile', help='证书文件')
    tls.add_argument('--keyfile', help='密钥文件')
    tls.add_argument('--keypass', help='密钥密码')
    ...
    if args.share and args.receive:
        ...
        if not args.arguments:
            ...
            raise FileNotFoundError(f'{args.arguments[0]} 不是一个文件夹')
            ...
            if not args.arguments:
                ...
                for f in args.arguments:
                    if not os.path.isfile(f):
                        raise FileNotFoundError(f'{f} 不是一个文件')
