...
class FileSendHandler(BaseHandler):
    ...
    def build_html(self, path, dirs, files):
        ...
        builder.append(f'<a href="/">{self.hostname}</a>')
        p = ''
        for name in path.split('/'):
            if name:
                name = html.escape(name)
                p = f'{p}/{name}'
                builder.append(f'&nbsp;/&nbsp;<a href="{p}/">{name}</a>')
        builder.append('</div>')
        builder.append('<hr>')
        ...
        for f, size in files:
            ...
            builder.append(f'&nbsp;<button class="btn_view" src="{f}">View</button>')
            builder.append('</div>')
        builder.append('</div>')
        builder.append('</div>')
        builder.append('</div>')
